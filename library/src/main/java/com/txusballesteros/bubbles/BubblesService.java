...
public class BubblesService extends Service {
    ...
    private void recycleBubble(final BubbleLayout bubble) {
        new Handler(Looper.getMainLooper()).post(new Runnable() {
            ...
            public void run() {
                if(bubble == null) {
//                    CommonLibrary.getIns().getApplication().reportCrash(new RuntimeException("bubble is null. "), "3037");
                }
                if (bubble != null && bubble.getWindowToken() != null) {
                    try {
                        getWindowManager().removeView(bubble);
                    } catch (IllegalArgumentException e) {
//                        CommonLibrary.getIns().getApplication().reportCrash(e, "3037");
                    }
                }
                BubbleLayout cachedBubble;
                for (Iterator<BubbleLayout> iterator = bubbles.iterator(); iterator.hasNext(); ) {
                    cachedBubble = iterator.next();
    ...
    private void recycleTrash() {
        if (bubblesTrash != null) {
            if (bubblesTrash.getWindowToken() != null) {
                try {
...
//                    CommonLibrary.getIns().getApplication().reportCrash(e, "3037");
    ...
    public synchronized void removeBubble(BubbleLayout bubble) {
        recycleBubble(bubble);
    }
